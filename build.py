...
class BuildManager:
    def __init__(self):
        # 경로 설정 - build.py가 프로젝트 루트에 있음
        self.script_dir = Path(__file__).parent.resolve()
        self.project_root = self.script_dir
        self.build_scripts_dir = self.project_root / "build_scripts"
        self.spec_file = self.build_scripts_dir / "video_downloader.spec"
        self.iss_file = self.build_scripts_dir / "setup_script.iss"
    ...
    def build_with_pyinstaller(self):
        ...
        try:
            # PyInstaller 실행
            cmd = ["pyinstaller", str(self.spec_file), "--clean", "-y"]
    ...
    def create_installer_package(self):
        ...
        try:
            ...
            print(f"실행 명령: {' '.join(cmd)}")
            result = subprocess.run(cmd, cwd=self.build_scripts_dir, check=True,
